      ...
      >
        <div>
          ...
          <p class="mt-4 flex items-center gap-4 text-muted">
            <NumberMotif
              v-if="story.original_publish_year"
              :text="story.original_publish_year"
            />
            <span><NumberMotif :text="formatTypeLabel(story.type)" /></span>
          </p>
          <p
            v-if="story.first_published_in"
        ...
        <p
          v-if="story.dark_tower_relation"
        ...
        <template #actions>
          <ShortStoryReadingActions
            :short-story-id="story.id"
            mode="expanded"
          />
        </template>
      </DetailHero>
