...
const localItems = ref<ReadingTimelineItem[]>([...props.items])
watch(
  () => props.items,
  (next) => {
...
function handleDeleted() {
  if (!editingEntry.value) return
  const readId = editingEntry.value.readId
  localItems.value = localItems.value.filter(entry => entry.readId !== readId)
}

function displayDate(entry: ReadingTimelineItem) {
  if (entry.readOn) {
...
<template>
  ...
  <div
    ...
    class="-mx-4 flex items-center gap-2 sm:mx-0"
  >
    <UButton
